...
class main(Tk):
    ...
    def widgets(self):
        ...
        Button(self, text="Quitter", command=self.terminate).pack() 
    def validate(self):
        if self.name_entry.get() != "":
            self.contact_list.update({self.name_entry.get().replace(",",""):[self.adress_entry.get().replace(",",""), self.phone_entry.get().replace(",",""), self.mail_entry.get().replace(",","")]})
        else:
    ...
    def show_contacts_details(self, name):
        ...
        self.details_window.geometry("350x150")
        for key in self.contact_list:
            if key == name:
